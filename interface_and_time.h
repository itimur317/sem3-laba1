...
void interface() {
    ...
    while (indikator) {
        ...
        if (indikator == 1) {
            ...
            ArraySequence<int> arr_seq3(seq1);                      // для 2 и 3 создадим те же массивы
            Sequence <int>* seq3 = &arr_seq3;

            double best_time = time_sort(seq1, bubble_sort);    // рассмотрим для бабл
            string best_name = "bubble sort";
            cout << "taken " << best_time << " microsecond by bubble sort" << endl;

            double time_for_merge = time_sort(seq2, merge_sort_for_seq);    // смотрим на мердж
            ...
            double time_for_BST = time_sort(seq3, sort_with_tree);      // смотрим на БСТ 
