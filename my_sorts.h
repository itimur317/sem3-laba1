...
void merge(Sequence<T>* seq, int (*cmp)(T, T), int left, int mid, int right) {
    int i, j, k;
    int left_leng = mid - left + 1;       // длины двух временных сиквенсов
...
void merge_sort(Sequence<T>* seq, int (*cmp)(T, T), int left, int right) {
    if (left < right) { // (l + r) / 2 == l + (r - l)/ 2 зато без проблем
        int mid = left + (right - left) / 2;  // делим пополам

        merge_sort(seq, cmp, left, mid);
        merge_sort(seq, cmp, mid + 1, right);    //продолжаем 

        merge(seq, cmp, left, mid, right);         // потихоньку мерджим
    }
}
